// pages/api/recommend-strategy.js
 ...
 * }
 */
async function getOnchainBalances(address, tokenList = []) {
  const rpc = process.env.AMOY_RPC_URL;
  if (!rpc) throw new Error("Missing AMOY_RPC_URL env var");
...
/*const functionSchema = {
  ...
  description: "Return a JSON portfolio allocation",
  parameters: {
    type: "object",
    properties: {
      riskProfile: { type: "string", description: "low|medium|high" },
      allocations: {
        type: "array",
        items: {
          type: "object",
...
const functionSchema = {
    name: "recommend_portfolio",
    description:
      "Return a JSON portfolio allocation including optional leveraged child-bet suggestions",
    parameters: {
      type: "object",
      properties: {
...
function buildContextText(risk, apyMap, balanceContext) {
  ...
  if (balanceContext) {
    ...
    if (tokenEntries.length) {
      ...
      for (const [id, t] of tokenEntries) {
        balanceTextParts.push(`  - ${id}: ${t.human} (${t.symbol || id})`);
      }
    }
  }
...
export default async function handler(req, res) {
  try {
    ...
    // NOTE: adjust to your OpenAI SDK usage; this uses a function-based tool call pattern similar to earlier
    const messages = [
      { role: "system", content: "You are a DeFi portfolio recommendation assistant. Output only valid JSON via the function call." },
      { role: "user", content: context }
    ];

    // The exact API shape depends on the OpenAI lib version; adapt if needed.
    ...
    let sum = allocs.reduce((s, a) => s + Number(a.percent || 0), 0);
    if (sum === 0 && allocs.length) {
      // equal split fallback
      const each = Math.floor(100 / allocs.length);
      allocs = allocs.map((a, i) => ({ ...a, percent: i === 0 ? 100 - each * (allocs.length - 1) : each }));
    } else if (sum !== 100) {
      // scale to 100
      allocs = allocs.map(a => ({ ...a, percent: Math.round((a.percent / sum) * 100) }));
      // fix rounding drift
      let s2 = allocs.reduce((x, y) => x + y.percent, 0);
